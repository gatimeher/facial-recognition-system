...
namespace registration
{
    public class AddFacultydll
    {
        ...
        {
            con = new SqlConnection("data source=DESKTOP-6RGINGV\\SQLEXPRESS;integrated security=true;database=FRSDatabase2;");
        }        
        public string saveFaculty(string fn, string add, string ph, string gen,string bc,string sub)
        {
            facultyname = fn;
            address = add;
            phone = ph;
            ...
            {
                con.Open();
                string qr = "insert into AddFacultyDB(Faculty_Name,Address,Phone_Number,Gender,Batch_Code,subject) values('" + facultyname + "','" + address + "','" + phone + "','" + gender + "','" + batchcode + "','"+subject+"')";
